...
print "Working on FastQC Stats Generation\n";
...
sub fastq_summary
{
	...
	for(my $fno=1; $fno<=$total_sub_fastq; $fno++) {
		...
		calculate_fastq_summary($working_dir."/".$sampleid."_1_".$fno.".fastq.gz",$working_dir."/".$sampleid."_2_".$fno.".fastq.gz",$fno,$working_dir,$maxlength,$sampleid);
		$manager->finish;
	}
...
sub calculate_fastq_summary {
	...
	#					Summary file
	#-------------------------------------------------------------------------------------------------------
	my $summary_file = "$working_dir/$sampleid.$fno".".summary.txt";
...
sub combine_summary {
	...
	open(OUTFP,">$working_dir/$sampleid.Read_Qual_File.txt") || die "Could not open $working_dir/$sampleid.Read_Qual_File.txt output file\n";
	print OUTFP "Percentage\tQuality\n";
	for(my $fno = 1; $fno <= $total_files; $fno++) {
		$input_file = "$working_dir/$sampleid".".$fno."."summary.txt";
		@records = ();
		$total_records = 0;
	...
	open(STATFP,">$working_dir/$sampleid.FastqSummary.txt") || die "Could not open statistics output file\n";
	...
	open(OUTFP1,">$working_dir/$sampleid.BaseComposition_R1.txt"); open(OUTFP2,">$working_dir/$sampleid.BaseComposition_R2.txt");
	open(OUTFP3,">$working_dir/$sampleid.BaseQualComposition_R1.txt"); open(OUTFP4,">$working_dir/$sampleid.BaseQualComposition_R2.txt");
